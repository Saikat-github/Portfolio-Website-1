      ...
      <div className='flex gap-4 sm:gap-8'>
        {
          socialLinks?.map((link, idx) => (
            <a href={link.link} target="_blank" rel="noopener noreferrer" key={idx}>
              <img src={link.logo} alt={link.name} className='w-10 h-10 border-2 border-white/20 bg-white/10 p-2 rounded-lg opacity-85 hover:opacity-70 transition-all duration-300' />
            </a>
          ))
        }
      </div>
